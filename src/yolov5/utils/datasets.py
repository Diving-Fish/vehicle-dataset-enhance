...
class LoadImages:  # for inference
    def __init__(self, path, frame_range, img_size=640):
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            if not ret_val or self.frame == self.frame_range[1]:
                self.count += 1
                self.cap.release()
            ...
            self.frame_progress = [self.frame, 0]
            if self.frame_range[1] == -1:
                self.frame_progress[1] = self.nframes
            else:
                self.frame_progress[1] = min(self.nframes, self.frame_range[1])
            self.frame_progress[0] -= self.frame_range[0]
            self.frame_progress[1] -= self.frame_range[0]
            caption = 'video %g/%g (%g/%g) %s: ' % (self.count + 1, self.nf, self.frame_progress[0],
                                                self.frame_progress[1], path)
        ...
        return path, img, img0, self.cap, caption
